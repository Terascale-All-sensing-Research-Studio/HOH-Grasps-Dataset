...
def process_grasps_plain_6d(translations, rotation_matrices, depths, widths, scores, quality_thresh=0.6, num_grasps=50):
    """
...
def load_grasps_plain_6d_valid(hoh_grasps_root, frame, scene_id, quality_thresh, num_grasps):
    """
    Load and demonstrate plain 6D style valid grasps for a given scene.
    ...
    grasp_label_path = os.path.join(scene_path, f'{scene_id}_grasps_valid_{frame}.npz')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--quality_thresh', type=float, default=0.6,
                        help='Quality threshold for grasps (higher scores for higher quality) [default: 0.6]')
    args = parser.parse_args()

    load_grasps_plain_6d_valid(args.hoh_grasps_root,
                               args.frame,
